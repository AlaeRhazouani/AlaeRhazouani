<p align="center">
  <a href="https://linkedin.com/in/alae-rhazouani"><img src="https://img.shields.io/badge/LinkedIn-alae--rhazouani-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="mailto:alae.g@icloud.com"><img src="https://img.shields.io/badge/Email-alae.g%40icloud.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="https://github.com/AlaeRhazouani"><img src="https://img.shields.io/badge/GitHub-AlaeRhazouani-181717?style=for-the-badge&logo=github&logoColor=white" /></a>
  <a href="https://github.com/AlaeRhazouani/AlaeRhazouani/blob/main/CV_Alae.pdf"><img src="https://img.shields.io/badge/Resume-Download_CV-f97316?style=for-the-badge&logo=adobeacrobatreader&logoColor=white" /></a>
  <a href="mailto:alae.g@icloud.com"><img src="https://img.shields.io/badge/Open_to-PFA_Internship_2026-f97316?style=for-the-badge&logoColor=white" /></a>
</p>

<h1 align="center">Hi there, I'm Alae Rhazouani</h1>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=22&pause=1000&color=2E9EF7&center=true&vCenter=true&width=900&lines=Software+Engineering+%26+Intelligent+Systems+Student;DevOps+%7C+Cloud+%7C+Infrastructure;Building+scalable%2C+automated%2C+and+reliable+systems" alt="Typing SVG" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=AlaeRhazouani&label=Profile%20views&color=0e75b6&style=flat" alt="AlaeRhazouani" />
</p>

<h3 align="center"><i>"Automate what can be automated, monitor what matters, and build systems that scale."</i></h3>

---

## About Me

2nd-year **Software Engineering & Intelligent Systems** student at **ENSAM Meknes**, specializing in DevOps & Cloud Engineering. I build CI/CD pipelines, containerized microservices, and cloud infrastructure — with hands-on experience deploying real systems to production. Currently deepening my knowledge in Kubernetes and infrastructure as code.

---

## Tech Stack

### DevOps & Cloud
<p>
  <img src="https://skillicons.dev/icons?i=docker,jenkins,linux,git,github" />
</p>

### Monitoring & Databases
<p>
  <img src="https://skillicons.dev/icons?i=prometheus,grafana,mysql,postgresql" />
</p>

### Languages & Frameworks
<p>
  <img src="https://skillicons.dev/icons?i=py,java,js,ts,dart,nodejs,express,flask,django,react,flutter" />
</p>

### Tools
<p>
  <img src="https://skillicons.dev/icons?i=figma,gitlab" />
</p>

---

## Current Focus

- Building and improving **CI/CD workflows** with Jenkins and Docker
- Learning more about **Kubernetes** and container orchestration
- Working with **Prometheus + Grafana** for observability
- Strengthening my skills in **DevOps best practices and IaC**

---

## Highlighted Projects

- **CI/CD Pipeline & Monitoring — Student Task Manager**
  Full CI/CD pipeline on Oracle Cloud — Jenkins Groovy DSL, Docker Compose, 3 Flask microservices, auto rollback, staging/production separation, Prometheus + Grafana observability. Everything triggered by a single `git push`.

- **AI Skills Matrix — CGI Maroc**
  AI portal analyzing software projects (ZIP + Word/PDF/Excel) to extract and map technical skills with proficiency levels and confidence scores. Built in active partnership with CGI Maroc.

- **Electric Meter OCR & QR Code**
  YOLOv8 OCR pipeline trained on a custom 2001-image dataset, detecting meter readings and QR codes in a single photo. React Native mobile app for field agents + admin dashboard + fully automated billing cycle with Dolibarr CRM sync.

- **Urban Docs Mobile App — FESURBAN**
  Cross-platform Flutter app replacing a fully paper-based process for consulting urban planning documents at the Agence Urbaine de Fès.

---

## GitHub Stats

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=AlaeRhazouani&theme=tokyonight" height="170" />
  <img src="https://streak-stats.demolab.com/?user=AlaeRhazouani&theme=tokyonight&hide_border=true" height="170" />
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=AlaeRhazouani&theme=tokyonight" height="170" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=AlaeRhazouani&theme=tokyo-night&hide_border=true" alt="Activity Graph"/>
</p>

---

<p align="center">
  <img src="https://raw.githubusercontent.com/platane/snk/output/github-contribution-grid-snake-dark.svg" alt="snake animation" />
</p>
